...
import { Clock, User, Users, AlertCircle, CheckCircle, X, Info, Calculator, Phone, Mail, Stethoscope, Save, Lock, Download, LogOut, FileText, Ban, Key, Trash2, Wifi, WifiOff, RefreshCw, Database, AlertTriangle, Send, ThumbsUp, Eraser, Power } from 'lucide-react';
...
// V3.3.7 Feature: System Shutdown Screen
const SystemClosedScreen = () => {
    return (
        <div className="fixed inset-0 bg-slate-900 z-[100] flex flex-col items-center justify-center text-white p-6 animate-in fade-in duration-500">
            <div className="w-28 h-28 bg-red-500/10 rounded-full flex items-center justify-center mb-8 ring-4 ring-red-500/20 shadow-[0_0_50px_rgba(239,68,68,0.3)]">
               <Power className="w-14 h-14 text-red-500 animate-pulse" />
            </div>
            <h1 className="text-4xl font-black mb-2 tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-orange-400">
                SYSTEM SHUTDOWN
            </h1>
            <h2 className="text-xl font-bold text-slate-300 mb-6">已過開放預約時間</h2>
            
            <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 max-w-md w-full backdrop-blur-sm shadow-xl">
                <p className="text-slate-400 text-center mb-4 leading-relaxed">
                    抱歉，現在時間已超過 <span className="text-white font-bold mx-1">17:00</span> (台灣時間)。<br/>
                    系統已自動進入休眠保護模式。
                </p>
                <div className="flex items-center justify-center space-x-2 text-xs font-mono text-slate-600 bg-black/20 p-2 rounded border border-slate-700/50">
                    <AlertTriangle size={12} className="text-yellow-600" />
                    <span>Error Code: 0x1700_TIMEOUT_S5</span>
                </div>
            </div>
...
const MassageBookingSystem = () => {
  ...
  // V3.3.7 State: System Shutdown
  const [isSystemClosed, setIsSystemClosed] = useState(false);

  // --- Firebase Logic ---
  
  // 1. Auth Initialization & Time Check
  useEffect(() => {
    ...
    // V3.3.7: Auto Shutdown Logic (Hardcoded Date Limit)
    // Wade Request: "2026/01/06 17:00 永久關掉"
    const checkTime = () => {
        // 設定死線：2026年1月6日 17:00:00 (台北時間 GMT+8)
        // ISO 8601 format with timezone offset ensuring precision
        const shutdownTimestamp = new Date('2026-01-06T17:00:00+08:00').getTime();
        const currentTimestamp = Date.now();
        
        // 只要現在時間超過死線，系統直接進入 S5 關機狀態
        if (currentTimestamp >= shutdownTimestamp) {
    ...
    const timer = setInterval(checkTime, 10000); // Check every 10 seconds (Polling)
  ...
  // V3.3.7: Priority Check - System Closed?
  if (isSystemClosed && !isAdminOpen) {
      // Allow admin bypass logic if needed, but per request, show closed screen to "others"
      // If Admin is logged in, we might still want to let them see data? 
      // For now, let's hard block unless they are already in admin view (isAdminOpen)
      return <SystemClosedScreen />;
  }
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h1 className="text-xl font-bold text-slate-800 leading-tight">住院醫師按摩預約系統 <span className="text-xs text-white bg-emerald-600 px-1.5 py-0.5 rounded ml-1">V3.3.7</span></h1>
